...
{
	gI_OffsetEffects = FindSendPropInfo("CBaseEntity", "m_fEffects");

	if (gI_OffsetEffects == -1)
	{
	...
	RegConsoleCmd("sm_showtriggers", CommandShowTriggers, "Command to dynamically toggle trigger visibility.");
	RegConsoleCmd("sm_st", CommandShowTriggers, "Command to dynamically toggle trigger visibility.");
}
...
void SetTriggerRenderColor(int entity)
{
	...
	if (strcmp(classname, "trigger_multiple") == 0)
	{
		...
		{
			GetOutputParameter(entity, "m_OnStartTouch", i, parameter);
		...
		for (int i = 0; i < count; i++)
		{
			GetOutputParameter(entity, "m_OnEndTouch", i, parameter);
...
void TransmitTriggers(bool transmit)
{
	...
	for (int entity = MaxClients + 1; entity <= GetEntityCount(); ++entity)
	{
		...
		// Get flags.
		int effectFlags = GetEntData(entity, gI_OffsetEffects);
		...
		// Apply state changes.
		SetEntData(entity, gI_OffsetEffects, effectFlags);
		ChangeEdictState(entity, gI_OffsetEffects);
