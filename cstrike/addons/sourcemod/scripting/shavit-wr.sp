...
public void OnMapStart()
{
	...
	char sQuery2[312];
	FormatEx(sQuery2, 312, "delete from `%splayertimes` where rowid not in (select min(rowid)from `%splayertimes` where map = '%s' group by auth,map,style)",gS_MySQLPrefix,gS_MySQLPrefix,gS_Map);
	gH_SQL.Query(SQL_RipDupes_Callback, sQuery2, 0, DBPrio_Low);
...
public void SQL_RipDupes_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (remove duplicates) SQL query failed. Reason: %s", error);
