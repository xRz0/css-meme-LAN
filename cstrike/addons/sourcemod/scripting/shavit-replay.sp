...
public void OnPluginStart()
{
	...
	gCV_BotShooting = new Convar("shavit_replay_botshooting", "1", "Attacking buttons to allow for bots.\n0 - none\n1 - +attack\n2 - +attack2\n3 - both", 0, true, 0.0, true, 3.0);
	...
	RegConsoleCmd( "sm_rewind", setreplay );
	RegConsoleCmd( "sm_skip", setreplay );
	// database
...
public int Native_GetReplayLength(Handle handler, int numParams)
{
	...
	return view_as<int>(GetReplayLength(style, GetNativeCell(2)));
}
...
public any Native_GetReplayTime(Handle handler, int numParams)
{
	...
	if(style==187)
		return float(gI_ReplayTick[style] - gA_FrameCache[style][track].iPreFrames) / gF_Tickrate;
	...
	if(central || iFrameCount > 0)
	{
		...
		else
		{
			...
			else
			{
				...
				char sTime[16];
				FormatSeconds((time == -1.0)? fWRTime:time, sTime, 16);
				Format(sName,sizeof(sName),"%s - %s",gA_FrameCache[style][track].sReplayName,sTime);
			}
		}
	}
	...
	if(GetClientTeam(client) > 1)
	{
		...
		else
		{
			if(Shavit_GetTimerStatus(client) == Timer_Running)
				xtime = Shavit_GetClientTime(client);
...
Action OpenReplayMenu(int client)
{
	...
	menu.AddItem("own", "Own Replay", (xtime >1.0)? ITEMDRAW_DEFAULT:ITEMDRAW_DISABLED);
...
public int MenuHandler_Replay(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		if(StrEqual(info, "own") && xtime > 1.0)
		{
			if(gA_CentralCache.iReplayStatus != Replay_Idle)
				StopCentralReplay(param1);

			int style = 187;
			if(gA_Frames[style][0]!=null)
				gA_Frames[style][0].Clear();

			gA_Frames[style][0] = gA_PlayerFrames[param1].Clone();
			int iSize = gA_Frames[style][0].Length;
			gA_FrameCache[style][0].iReplayVersion = REPLAY_FORMAT_SUBVERSION;
			gA_FrameCache[style][0].iFrameCount = iSize - gI_PlayerPrerunFrames[param1];
			gA_FrameCache[style][0].iPreFrames = gI_PlayerTimerStartFrames[param1] - gI_PlayerPrerunFrames[param1];
			gA_FrameCache[style][0].fTime = xtime;
			char sName[MAX_NAME_LENGTH];
			GetClientName(param1, sName, MAX_NAME_LENGTH);
			ReplaceString(sName, MAX_NAME_LENGTH, "#", "?");
			gA_FrameCache[style][0].bNewFormat = true;
			strcopy(gA_FrameCache[style][0].sReplayName, MAX_NAME_LENGTH, sName);
			
			gI_ReplayTick[style] = 0;
			gI_TimerTick[style] = style;
			gA_CentralCache.iStyle = style;
			gA_CentralCache.iTrack = 0;
			gA_CentralCache.iPlaybackSerial = GetClientSerial(param1);
			gF_LastInteraction[param1] = GetEngineTime();
			gI_ReplayBotClient[style] = gA_CentralCache.iClient;
			gRS_ReplayStatus[style] = gA_CentralCache.iReplayStatus = Replay_Start;
			TeleportToStart(gA_CentralCache.iClient, style, 0);
			gB_ForciblyStopped = false;
			UpdateReplayInfo(gA_CentralCache.iClient, style, xtime, 0);
		...
		{
			OpenReplaySubMenu(param1, param2-1);
		}
	}
...
public Action setreplay(int client, int args)
{
	if(client != 0)
	{
		if(gA_CentralCache.iReplayStatus != Replay_Running)
		{
			Shavit_PrintToChat(client,"You should start a replay before you use this command!");
			return Plugin_Handled;
		}
		...
		char info1[8];
		char info2[8];
		float prefloat = GetConVarFloat(gCV_PlaybackPreRunTime);
		int pretime = RoundToZero(prefloat*gF_Tickrate);

		if (args == 1)
		{
			GetCmdArg(1, info, sizeof(info));
			int time = StringToInt(info);
			int frametime = RoundToZero(time*gF_Tickrate)+pretime;
			if(frametime > gA_FrameCache[gA_CentralCache.iStyle][gA_CentralCache.iTrack].iFrameCount || frametime < 1)
			{
				Shavit_PrintToChat(client,"The replay time is faster... Your input isn't valid at all!");
				return Plugin_Handled;
			}
			gI_ReplayTick[gA_CentralCache.iStyle] = frametime;
			Shavit_PrintToChat(client,"Replay set to: %ss!",info);
			PrintToConsole(client,"Replay set to: %ss!",info);
			return Plugin_Handled;
		}
		...
		else if(args == 2)
		{
			...
			int time2 = StringToInt(info1);
			int frametime = RoundToZero(((time*60)+time2)*gF_Tickrate)+pretime;
			if(frametime > gA_FrameCache[gA_CentralCache.iStyle][gA_CentralCache.iTrack].iFrameCount || frametime < 1)
			{
				Shavit_PrintToChat(client,"The replay time is faster... Your input isn't valid at all!");
				return Plugin_Handled;
			...
			gI_ReplayTick[gA_CentralCache.iStyle] = frametime;
			Shavit_PrintToChat(client,"Replay set to: %sm %ss!",info,info1);
			PrintToConsole(client,"Replay set to: %sm %ss!",info,info1);
			return Plugin_Handled;
		}
		...
		else if(args == 3)
		{
			...
			GetCmdArg(2, info1, sizeof(info1));
			int time2 = StringToInt(info1);
			GetCmdArg(3, info2, sizeof(info2));
			int time3 = StringToInt(info2);
			int frametime = RoundToZero(((time*3600)+(time2*60)+time3)*gF_Tickrate)+pretime;
			if(frametime > gA_FrameCache[gA_CentralCache.iStyle][gA_CentralCache.iTrack].iFrameCount || frametime < 1)
			{
				Shavit_PrintToChat(client,"The replay time is faster... Your input isn't valid at all!");
				return Plugin_Handled;
			...
			gI_ReplayTick[gA_CentralCache.iStyle] = frametime;
			Shavit_PrintToChat(client,"Replay set to: %sh %sm %ss!",info,info1,info2);
			PrintToConsole(client,"Replay set to: %sh %sm %ss!",info,info1,info2);
			return Plugin_Handled;
		}
		else
		{
			Shavit_PrintToChat(client,"Command usage: !rewind <Hours> <Minutes> <Seconds>");
			return Plugin_Handled;
		}
	}
	return Plugin_Handled;
