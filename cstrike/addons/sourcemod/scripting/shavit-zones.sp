...
public void OnPluginStart()
{
	...
	// menu
	RegConsoleCmd("sm_setstart", Command_SetStart, "Set your !r position.");
...
public Action Command_SetStart(int client, int args)
{
	...
	if(Shavit_InsideZone(client,Zone_Start,Track_Main))
	{
		GetClientAbsOrigin(client, startPos[0]);
		GetClientEyeAngles(client, startPos[1]);
		startposSet[0] = true;
		Shavit_PrintToChat(client, "Main Start Position set!");
		return Plugin_Handled;
	}
	if(Shavit_InsideZone(client,Zone_Start,Track_Bonus))
	{
		GetClientAbsOrigin(client, startPos[2]);
		GetClientEyeAngles(client, startPos[3]);
		startposSet[1] = true;
		Shavit_PrintToChat(client, "Bonus Start Position set!");
		return Plugin_Handled;
	}
		
	Shavit_PrintToChat(client, "Be inside a start zone to set your start position!");
...
public void Shavit_OnRestart(int client, int track)
{
	if(gCV_TeleportToStart.BoolValue)
	{
		if(startposSet[0])
		{
			TeleportEntity(client, startPos[0], startPos[1], view_as<float>({0.0, 0.0, 0.0}));
			Shavit_StartTimer(client, track);
			return;
		}
		if(startposSet[1])
		{
			TeleportEntity(client, startPos[2], startPos[3], view_as<float>({0.0, 0.0, 0.0}));
			Shavit_StartTimer(client, track);
			return;
		}
