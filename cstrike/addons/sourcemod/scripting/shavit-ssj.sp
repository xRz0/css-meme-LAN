...
public Plugin myinfo =
{
	name = "SSJ: Advanced",
	author = "AlkATraZ",
	description = "Strafe gains/efficiency etc for shavit's timer",
...
public void OnPluginStart()
{
	...
	g_hCookieStrafeSync = RegClientCookie("ssj_strafesync", "ssj_strafesync", CookieAccess_Public);
	g_hCookieDefaultsSet = RegClientCookie("ssj_defaultz", "ssj_defaultz", CookieAccess_Public);
	ssjprinte_Cookie = RegClientCookie("ssj_printz", "SSJ print settingz", CookieAccess_Protected);
...
public void OnClientCookiesCached(int client)
{
	...
	if(StringToInt(sCookie) == 0)
	{
		...
		printset[client] = "cchon";
		SetClientCookie(client, ssjprinte_Cookie, printset[client]);
	}
	...
	GetClientCookie(client, ssjprinte_Cookie, sCookie, 8);
	printset[client] = sCookie;
}
...
Action ShowSSJMenu(int client, int item = 0)
{
	...
	if(StrEqual(printset[client], "chat")) //XD
	{
		menu.AddItem("pset", "[Chat] Print Settings");
	}
	if(StrEqual(printset[client], "cons"))
	{
		menu.AddItem("pset", "[Console] Print Settings");
	}
	if(StrEqual(printset[client], "cchon"))
	{
		menu.AddItem("pset", "[Chat+Console] Print Settings");
	}
	if(StrEqual(printset[client], ""))
	{
		menu.AddItem("pset", "[Chat+Console] Print Settings");
	}
...
void PrintToClient(int client, const char[] message, any...)
{
	...
	if(g_bShavit)
	{
		if(StrEqual(printset[client], "chat"))
		{
			Shavit_StopChatSound();
			Shavit_PrintToChat(client, "%s", buffer); // Thank you, GAMMACASE
		}
		else if(StrEqual(printset[client], "cons"))
		{
			ReplaceString(buffer, sizeof(buffer), gS_ChatStrings.sText, "", false);
			ReplaceString(buffer, sizeof(buffer), gS_ChatStrings.sVariable, "", false);
			PrintToConsole(client, "%s", buffer);
		}
		else if(StrEqual(printset[client], "cchon"))
		{
			Shavit_StopChatSound();
			Shavit_PrintToChat(client, "%s", buffer); // Thank you, GAMMACASE
			ReplaceString(buffer, sizeof(buffer), gS_ChatStrings.sText, "", false);
...
void printssjwhere(int client)
{
	Handle ssjprint = CreateMenu(MenuHandler_ssjprintMenuz, MENU_ACTIONS_ALL);
	SetMenuTitle(ssjprint, "~ SSJ print position ~ \n ");

	AddMenuItem(ssjprint, "chat", "chat");
	AddMenuItem(ssjprint, "cons", "console");
	AddMenuItem(ssjprint, "cchon", "chat + console");
	AddMenuItem(ssjprint, "back", "Back to SSJ menu");
	SetMenuExitButton(ssjprint, true);

	DisplayMenu(ssjprint, client, 50);
}

public int MenuHandler_ssjprintMenuz(Handle ssjmenuu, MenuAction maAction, int client, int choice)
{
	if(maAction == MenuAction_Select)
	{
		char sChoice[8];
		GetMenuItem(ssjmenuu, choice, sChoice, 8);

		if(StrEqual(sChoice, "chat"))
		{
			SetClientCookie(client, ssjprinte_Cookie, sChoice);
			printset[client] = sChoice;
			printssjwhere(client);
			PrintToClient( client, "%s SSJ is showing in %schat%s.", gS_ChatStrings.sText,gS_ChatStrings.sVariable,gS_ChatStrings.sText);
		}

		if(StrEqual(sChoice, "cons"))
		{
			SetClientCookie(client, ssjprinte_Cookie, sChoice);
			printset[client] = sChoice;
			printssjwhere(client);
			PrintToClient( client, "%s SSJ is showing in %sconsole%s.", gS_ChatStrings.sText,gS_ChatStrings.sVariable,gS_ChatStrings.sText);
		}

		if(StrEqual(sChoice, "cchon"))
		{
			SetClientCookie(client, ssjprinte_Cookie, sChoice);
			printset[client] = sChoice;
			printssjwhere(client);
			PrintToClient( client, "%s SSJ is showing in %sconsole %s+ %schat%s.",gS_ChatStrings.sText,gS_ChatStrings.sVariable,gS_ChatStrings.sText,gS_ChatStrings.sVariable,gS_ChatStrings.sText);
		}

		if(StrEqual(sChoice, "back"))
		{
			ShowSSJMenu(client);
		}
